...
The local database is an Apache Derby in-memory database.
...
[id="plugins-{type}s-{plugin}-multi-pipelines"]
[IMPORTANT]
===============================
Read the next section when using this plugin in with multiple
pipelines.
===============================
These considerations are necessary because the lookup
database, an in-memory Apache Derby instance, is a single
instance for the whole JVM.

===== Same lookup enhancement
The big concern here is the timing of pipeline creation. As
each pipeline is created during Logstash startup, each plugin
will begin creating the local objects and filling the local
tables with remote data. It is possible to leave the loader
and db_object setting empty on all but one pipeline config -
but that runs the risk of events being processed by the
lookup only configs before the loader fills the local db with
remote data. To remove this risk, all pipelines can define
loaders and db_objects (use the `preserve_existing` option).
In this case all the loaders will fill the local db. This
has been tested to work without error. Be careful if using
scheduled loading, make sure the configs have an interleaved
schedule.

===== Different lookup enhancement
The loaders, db_objects and lookups should reference
independent local tables to ensure that there is no conflicts
when the plugins from each pipeline modify the local database.
