...
filter {
  jdbc_static {
    ...
    import_file_temp_directory => "/tmp/logstash/jdbc_static/import_data"
    jdbc_user => "logstash"
...
| <<plugins-{type}s-{plugin}-import_file_temp_directory>> |<<string,string>>|No
...
JDBC driver library path to third-party driver library. If multiple
libraries are required, pass them as a comma-separated list. 
...
[id="plugins-{type}s-{plugin}-import_file_temp_directory"]
===== `import_file_temp_directory`
